...
if (!empty($id) && !empty($cod_animal)) {
    $resultado = $controller->read($id);
} else {
    echo "ID ou código de animal não foram fornecidos na URL.";
    // Lide com esse caso de erro aqui, redirecionando para outra página ou exibindo uma mensagem de erro.
    // Por exemplo, com header("Location: pagina_de_erro.php");
}
      ...
      <div class="container">
        ...
        <div class="max-width">
          <div class="imageContainer">
            <?php if (isset($resultado['imagem_perfil']) && file_exists($resultado['imagem_perfil'])) { ?>
                <img src="<?php echo $resultado['imagem_perfil']; ?>" alt="Imagem do Anfitrião" class='imagem-100x100'>
            <?php } else { ?>
                <img src="img/perfil.png" id="imgPhoto" alt="Imagem padrão" class='imagem-150x150'>
            <?php } ?>
          </div>
        </div>
        <!--fim-->
        ...
        <br>
        <p>Complemento: <?php echo $resultado['complemento']; ?></p>
        <p>Preferencias: <?php echo $resultado['preferencias']; ?></p>
        <br>
        <p>Valor Hora: <?php echo $resultado['valor_hora']; ?></p>
      </div>



      <div class="container2">
        <form method="post" action="../Controllers/HospedagemController.php?funcao=create&cod_anfitriao=<?php echo $resultado['cod']; ?>&cod_tutor=<?php echo $_SESSION['tutor_cod']; ?>&cod_animal=<?php echo $cod_animal; ?>">
            <div class="desc">
              <div class="heading">
                <h3>Agendar Hospedagem - Data e hora</h3>
              </div>
              <br>
             <h6> Inicio da Hospedagem </h6>
              <div class="input-wrap">
                <input class="input-field" type="datetime-local" name="dia_hosp">
              </div>
              <br>
              <h6> Fim da Hospedagem </h6>
              <div class="input-wrap">
                <input class="input-field" type="datetime-local" name="dia_busca">
              </div>
