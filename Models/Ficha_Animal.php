...
class Ficha_Animal extends Conexao {
	...
	public function create($ficha, $cod_tutor) {
		try {
			...
			$stmt->bind_param("ssssssssssss", $cod_tutor, $ficha->nome, $ficha->especie, $ficha->raca, $ficha->sexo, $ficha->idade, $ficha->peso, $ficha->tamanho, $ficha->caracteristicas, $ficha->comportamento, $ficha->historico_medico, $ficha->instrucoes_especiais);
	...
	public function listarAnimaisPorTutor($tutorCod) {
		...
		try {
			$stmt = $this->mysqli->prepare("SELECT * FROM ficha_animal WHERE cod_tutor = ?");
