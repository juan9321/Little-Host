...
class Hospedagem extends Conexao {
	...
	public function create(){
		try {
			$stmt = $this->getConexao()->prepare("INSERT INTO hospedagem (`cod_tutor`, `cod_animal`, `cod_anfitriao`, `data_hosp`, `data_busca`) VALUES (?, ?, ?, ?, ?);");
	
			if ($stmt === false) {
				die("Erro na preparação da consulta: " . $this->getConexao()->error);
			}
	
			$stmt->bind_param("iiiss", $this->cod_tutor, $this->cod_animal, $this->cod_anf, $this->dia_hosp, $this->dia_busca);
			...
			echo "Ocorreu um erro ao realizar a hospedagem: " . $e;
		}
	}
