...
class Anfitriao extends Conexao {
		...
		try {
            ...
            if($total > 0){
				...
				foreach ($lista as $l) {
					...
					$anfitriao['imagem_perfil'] = $l['imagem_perfil'];
		...
		try {
            ...
            if($total > 0){
				...
				foreach ($lista as $l) {
					...
					$anfitrioes[$i]['imagem_perfil'] = $l['imagem_perfil'];
					$i++;
				}
	...
	{
		try {
            $stmt = $this->mysqli->query("DELETE imagem_perfil FROM anfitriao WHERE `cod` =  " . $id . ";");
